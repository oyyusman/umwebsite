...
if (isset($_POST['add_room'])){
  ...
  if(insert($q1,$values,'siiiiis')){
        $flag=1;
            ...
            //   new method to prevent execution of time
                //  when we selected five faclities then five will be added to the that room 
            //  to that room that means room id aare repeated but facilities are different
        //  therefore we use prepared stateemt here to minimize execution time
...
if(isset($_POST['get_room'])){
  ...
  if(mysqli_num_rows($res2)>0){
    // fetching data from other 2 tables room_facilities and room features
  ...
  while($row=mysqli_fetch_assoc($res)){
    ...
    echo<<<data
        ...
        <tr>
          ...
          <button type='button' onclick='rem_image($row[sr_no],$row[room_id])' class='btn btn-primary shadow-none btn-sm' >
             <i class='bi bi-pencil-square'></i> Edit
          </button>
...
if(isset($_POST['remove_room'])){
  $frm_data=filteration($_POST);
  $res_1=select("SELECT * FROM `room_images` WHERE `room_id`=?",[$frm_data['room_id']],'i');
  while($row=mysqli_fetch_assoc($res_1)){
    deleteImage($row['image'],ROOMS_FOLDER);
  }
  $res_2=delete("DEELTE FROM `room_images` WHERE `room_id`=? " ,[$frm_data['room_id']],'i');
  $res_3=delete("DEELTE FROM `room_features` WHERE `room_id`=? " ,[$frm_data['room_id']],'i');
  $res_4=delete("DEELTE FROM `room_facilities` WHERE `room_id`=? " ,[$frm_data['room_id']],'i');
  $res_5=update("UPDATE `rooms` SET `removed`=?  WHERE ``" ,[1,$frm_data['room_id']],'ii');
